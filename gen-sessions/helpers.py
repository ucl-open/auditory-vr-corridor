...
MAX_SHAPING_STAGE = 6
# Highest stage reached by advancing through the stages. Stage 6 adds probe trials and is only ever chosen by hand, but it is kept once
...
def determine_shaping_stage(
    ...
    - <70% success: stay in current stage
    ...
    # If at least 50 trials, calculate success rate and determine shaping stage for next session
        ...
        if end_stage in range(3, MAX_AUTO_STAGE):
        ...
        if success_rate >= 0.7:
            if end_stage >= MAX_AUTO_STAGE:
                print(f"\nSuccess rate >= 70%, but stage {end_stage} is not advanced past automatically. Staying in current shaping stage.")
                next_stage = end_stage
            else:
